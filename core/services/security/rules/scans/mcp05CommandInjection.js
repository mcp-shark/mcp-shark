...
const RULE_ID = 'mcp05-command-injection';
const OWASP_ID = 'MCP05';
const RECOMMENDATION =
  'Implement strict input validation and sanitization. Use parameterized commands and avoid shell execution.';
...
export function scanMCP05CommandInjection(mcpData = {}) {
  const results = {
    toolFindings: [],
    resourceFindings: [],
    promptFindings: [],
  ...
  for (const tool of mcpData.tools || []) {
    ...
    if (scan) {
      results.toolFindings.push({
        ...
        safeUseNotes:
          'Review tool input validation. Ensure no shell command execution without proper sanitization.',
      });
    }
  }
...
const adapter = createRuleAdapter(scanMCP05CommandInjection, RULE_ID, OWASP_ID, RECOMMENDATION);
    ...
    convertPacketFinding(
      {
        issueType: 'Command Injection',
        severity: 'critical',
        title: 'Command Injection Pattern in Traffic',
        description: `Potential command injection in packet: ${scan.matches.join(', ')}`,
...
export const ruleMetadata = {
  ...
  owasp_id: OWASP_ID,
  severity: 'critical',
  description: 'Detects command injection patterns that could lead to remote code execution.',
  source: 'static',
  type: 'owasp-mcp',
};
