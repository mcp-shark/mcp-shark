 ...
 */
import { hookToolsListResponse } from '#llm/drift/hook.js';
 ...
 */
async function logResponsePacket(db, options) {
  const {
    statusCode,
    headers = {},
    body,
  ...
  // Hook into tools/list responses for drift tracking
  // Note: Drift tracking is optional, errors are caught and logged but don't block packet logging
  if (jsonrpcMethod === 'tools/list' && jsonrpcResult && statusCode >= 200 && statusCode < 300) {
    const serverKeyForDrift = serverKey || null;
    if (serverKeyForDrift) {
      hookToolsListResponse(db, frameNumber, jsonrpcMethod, jsonrpcResult, serverKeyForDrift).catch(
        (err) => {
          logger.error('Drift tracking hook failed', {
            error: err.message,
            frameNumber,
            serverKey: serverKeyForDrift,
          });
        }
      );
    }
